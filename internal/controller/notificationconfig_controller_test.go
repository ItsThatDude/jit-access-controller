...
var _ = Describe("NotificationConfig Controller", func() {
	Context("When reconciling a resource", func() {
		...
		notificationconfig := &accessv1alpha1.NotificationConfig{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind NotificationConfig")
			err := k8sClient.Get(ctx, typeNamespacedName, notificationconfig)
			if err != nil && errors.IsNotFound(err) {
				resource := &accessv1alpha1.NotificationConfig{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance NotificationConfig")
