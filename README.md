...
```sh
make docker-build docker-push IMG=<some-registry>/jit-access:tag
```
...
```sh
make deploy IMG=<some-registry>/jit-access:tag
```
...
#### Build the installer for the image built and published in the registry
  ...
  ```sh
  make build-installer IMG=<some-registry>/jit-access:tag
  ```
...
#### Using the installer
  ...
  ```sh
  kubectl apply -f https://raw.githubusercontent.com/<org>/jit-access/<tag or branch>/dist/install.yaml
  ```
